# 01 · Documento Maestro — Lazarus (defensa contra prompt injection por diseño)

> Documento central del proyecto Lazarus. Consolida origen, problema, modelo de amenaza, arquitectura, decisiones, alcance del MVP, stack y fuentes.
...
1. Entra a su cuenta en **Lazarus** y **conecta sus MCPs** (Gmail, calendario, CRM, base de datos, etc.). Las **credenciales/keys de cada MCP se gestionan desde la UI** —la misma experiencia de conectar un MCP que ya conoce de su proveedor— y quedan custodiadas por el gateway. Lazarus los introspecta y arma un **catálogo de tools tipadas**.
...
4. En su proveedor LLM (Claude, ChatGPT, su propio agente), **quita todos los MCPs sueltos** y deja **solo el MCP de Lazarus**.
5. Sigue trabajando igual. Por dentro, cada tarea pasa por la defensa de Lazarus. La UI le muestra trazas: qué tools se llamaron, qué datos fluyeron, qué se bloqueó.
...
- La `description` del tool está **cuidadosamente redactada** (ver §7.5) para instruir al agente externo: qué hace Lazarus, qué fuentes/capacidades existen, y —crítico— que **el requerimiento debe ser COMPLETO, no parcial**: toda la intención de la tarea debe ir en una sola llamada, porque el plan se fija de una vez y el sistema no itera pidiendo aclaraciones sobre datos.
...
- **Decisión:** intérprete **propio, escrito desde cero**, hecho a medida para Lazarus. No partimos de ninguna base externa: queremos control total del sistema de tipos y del tracking de capacidades, sin arrastrar deuda de código ajeno, y libertad para elegir el stack óptimo (§8).
...
- **Decisión:** las keys/credenciales de cada MCP upstream se **introducen y gestionan desde la UI de Lazarus** (formulario por servidor, secreto guardado, conexión verificable). El gateway las custodia; el agente externo **nunca** las ve: solo habla con `run_task`. Esto consolida el secreto en un punto con políticas.
