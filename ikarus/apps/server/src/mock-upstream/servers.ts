...
/** Mock "Gmail": a read-only mailbox returning the demo inbox (§11). */
...
export interface MailerServer {
  server: McpServer;
  /** Recorded send attempts. In the demo, a blocked send never lands here. */
  sent: Array<{ to: unknown; body: unknown }>;
}

/** Mock mailer: a sink that only RECORDS the attempt (sends nothing real). */
export function createMailerServer(): MailerServer {
  const sent: MailerServer["sent"] = [];
