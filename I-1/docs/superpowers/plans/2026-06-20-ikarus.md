...
**Interfaces:**
- Consumes: everything above, plus `ikarus.p_llm` (`plan`, `build_catalog`), `ikarus.llm_client` (`LLMClient`), `ikarus.config` (`load_settings`).
- Produces:
  - `run_scene(scene: int, scenario_name: str, mock: bool = True, client: LLMClient | None = None) -> dict` — returns `{"text": str, "blocked": bool, "executed_sinks": list, "used_fallback": bool, "naive_recipient": str | None}`.
    - Scene 1: choose the plan by mode (**hybrid live wiring**): in `mock=True`, use `canonical_plan`. In `mock=False`, call `p_llm.plan(request, build_catalog(registry), canonical=canonical_plan, client=...)` so the **real P-LLM emits the plan**; `used_fallback` reflects the `PlanResult`. The P-LLM sees ONLY request + catalog. Then run the plan through the interpreter.
    - Scene 2: ALWAYS run `tainted_plan` (the deliberately-subverted, defense-in-depth case — the planner would never emit this; mode does not change it). Recipient is extracted UNTRUSTED ⇒ blocked.
    - Scene 3: run `naive_agent`; hijacked to attacker.
    - Q-LLM extraction stays deterministic mock in all modes (hybrid decision — see Task 9 / HONESTY.md).
    - `client` is injectable for testing; when `mock=False` and `client is None`, build one from `load_settings()`.
  - `main(argv: list[str] | None = None) -> int` — argparse: `--scene {1,2,3,all}`, `--scenario {email,pdf}`, `--mock/--live`. Prints rendered text and, in live mode, a fallback note when `used_fallback` is true. Returns exit code 0.
